...
AWS.SharedIniFileCredentials = AWS.util.inherit(AWS.Credentials, {
  ...
  loadRoleProfile: function loadRoleProfile(creds, roleProfile, callback) {
    ...
    */
    if (!sourceProfileName && !credentialSource) {
      ...
      if (sourceProviders.hasOwnProperty(cred))
      for (var i = 0, sourceProviderNames = Object.keys(sourceProviders); i < sourceProviderNames.length; i++) {
        var sourceProviderName = sourceProviderNames[i];

        if (credentialSourceName === sourceProviderName || credentialSourceName.toLowerCase() === sourceProviderName.toLowerCase()){
   ...
   * @api private
   */
  resolveCredentialsFromSource: function resolveCredentialsFromSource(credentialSourceName) {
